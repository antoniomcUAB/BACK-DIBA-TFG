...
public class DataBaseInit implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		SituacioSocial situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Manca d'autonomia personal a les  activitats bàsiques de la vida diaria  (A.1)",
																						"Situació en qué la persona no pot desenvolupar les activitats bàsiques de la vida diaria (alimentació, higiene, seguiment médic i farmacològic, mobilitat personal, descans, etc.), independentment de la causa que la motiva: malaltia física o psíquica, trastorn mental, deteriorament cognitiu, discapacitat física, psíquica,  intelélectual o sensorial, etc.",entornAutonomia));
		
		FrequenciaGravetat frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Necessita suport/atencié/supervisié en algun moment puntual de la setmana/dia",baixaGravetat));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Necessita suport/atencié/supervisié per les activitats de la vida diéria",moderadaGravetat)); 
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(
									new FrequenciaGravetat(situacioSocial,
											"Necessita atencié/supervisié continua les 24h",
											moderadaGravetat)); 
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Necessita atencié/supervisié continua les 24h",altaGravetat));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Necessita suport/atencié/supervisié en algun moment puntual de la setmana/dia",moderadaGravetat));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"situació impagament prévia a léinici del procés judicial. Ocupacié il·legall",baixaGravetat));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Diverses famélies amb menors o dependents compartint espais. Situacions de relloguers sense contracte.",altaGravetat));
		criteriRepository.save(new Criteri ("Des de fa menys de 3 mesos i de manera no repetida en el temps",frequentFrequencia,vulnerabilitatRisc,frequenciaGravetat));
		criteriRepository.save(new Criteri ("Fa més de 3 mesos",puntualFrequencia,alttRisc,frequenciaGravetat));
								
								
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Risc de pérdua o manca serveis/subministraments (H.5)","Risc de perdua o manca de subministraments (gas, aigua, electricitat), manca de parament bésic o estris doméstics imprescindibles per satisfer les necessitats bésiques de la unitat familiar. Inclou els risc de perdua per connexió il·legall.",entornHabitatge));
		...
		criteriRepository.save(new Criteri ("Segon cop durant léany",continuaFrequencia,vulnerabilitatRisc,frequenciaGravetat));
		...
		criteriRepository.save(new Criteri ("Segon cop durant léany",continuaFrequencia,riscRisc,frequenciaGravetat));
		criteriRepository.save(new Criteri ("Més de 2 cops durant any",puntualFrequencia,riscRisc,frequenciaGravetat));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Disposen de xarxa o recursos que proporcionen  suport econémic. No han déafrontar pagament de léhabitatge habitual",moderadaGravetat));
		...
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Administracié deficient (E.3)","Unitat familiar amb ingressos però que presenta dificultats per administrar el pressupost  i no prioritzar la cobertura de les necessitats bésiques.",entornEconomic));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Degut a la mala gestié tenen dificultats per cobrir les despeses bésiques de la unitat familiar ",moderadaGravetat));
		criteriRepository.save(new Criteri ("Situacions concretes  i capacitat de canvi",continuaFrequencia,vulnerabilitatRisc,frequenciaGravetat));
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Voluntat de treballar però dificultat en léaccés al mercat de treball. Situacions sobrevingudes (salut, situació legal en trémit, etc..)",baixaGravetat));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Famélia que entorpeix la intervencié social ni accepta el treball. Afectacions molt greus en el desenvolupament integral de léinfant o adolescent",altaGravetat));				
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Famélia que entorpeix la intervencié social ni accepta el treball. Afectacions molt greus en el desenvolupament integral de léinfant o adolesecent. Comportaments agressius envers companys o mestres",altaGravetat));				
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial," No hi ha voluntat de canvi ni millora, La famélia incentiva o no colélabora per resoldre la situació. Afectacions molt greus en el desenvolupament integral de léinfant o adolesecent. Comportaments agressius envers companys o mestres",altaGravetat));				
																		
		criteriRepository.save(new Criteri ("Sense valoració",senseFrequencia,alttRisc,frequenciaGravetat));		
																
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Absentisme escolar (ES.4)","No assisténcia al centre en que esta matriculat, de forma reiterada i no justificada, de léalumnat en edat escolar (dels 3 als 16 anys) ja sigui per prépia voluntat o per voluntat de la famélia.",entornEscolar));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"La famélia incentiva o no colélabora per resoldre l'absentisme",altaGravetat));		
		...
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Assetjament escolar (ES.5)","Infant o adolescent que esté patint Situacions habituals de violéncia física i psíquica en el marc de léescola, o que séha iniciat en aquest marc encara que les actuacions es produeixin fora, per part dels seus companys i companyes, ja sigui en l'entorn escolar o fora déell",entornEscolar));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Sense perill fésic i léafectacié a nivell pséquic/emocional no interfereix en les competéncies bésiques de la persona (autocura, comunicacié, autonomia i iniciativa)",moderadaGravetat));					
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial," Manca de lémits adequats a léedat dels infants/adolescents/. No séinvolucra activament en el desenvolupament/cura de la persona. Un altre adult assumeix les responsabilitats.Voluntat de canvi i capacitat de millora",baixaGravetat));		
																		
		criteriRepository.save(new Criteri ("Sense valoració",senseFrequencia,vulnerabilitatRisc,frequenciaGravetat));			
																
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Manca de lémits  o de cura de les persones que en depenen i ningé dins del nucli de convivéncia assumeix les responsabilitats. Voluntat de canvi i capacitat de millora",moderadaGravetat));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Instaurat en les dinémiques familiars sense que  interfereixi en les activitats bésiques.",moderadaGravetat));		
		...
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Sobre-responsabilitat (F.6)","Membre de la unitat de convivéncia, adult o menor, que assumeix responsabilitats familiars que es considera que es troben per sobre de les seves possibilitats. Aqué quedaria inclés léembarés adolescent Amb menors cal tenir en compte: Léactivitat causant de la sobre-responsabilitat- La maduresa i edat de léinfant. - La maduresa de la famélia (si la famélia vetlla o no per minimitzar léimpacte de la responsabilitat atribuéda al menor).- Si disposa de la supervisié déalgun adult de l'entorn de proximitat.- El temps que séexposa a la sobre-responsabilitat",entornFamiliar));		
											
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"Relacionat a fets concrets sobrevinguts.",baixaGravetat));		
																		
		criteriRepository.save(new Criteri ("Sense valoració",senseFrequencia,vulnerabilitatRisc,frequenciaGravetat));				
																
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"La situació afecta a un adult amb mancances però amb capcitat de donar resposta a la situació mitjanéant el suport necessari. La situació afecta a un/a menor major de 16 anys, sense que afecti greument a la seva integritat piscolégica i amb possiblitat de canvis i millora.",moderadaGravetat));		
		...
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"situació puntual i reversible amb el suport adequat. Persones diagnosticades que colélaboren.",baixaGravetat));		
																		
		criteriRepository.save(new Criteri ("Sense valoració",senseFrequencia,vulnerabilitatRisc,frequenciaGravetat));				
																
		frequenciaGravetat = frequenciaGravetatRepository.save(new FrequenciaGravetat(situacioSocial,"situació reiterada amb necessitat de suport extern. Persones diagnosticades que no colélaboren",moderadaGravetat));		
		...
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Dificultats en les relacions socials i a la participacié a la comunitat (S.1)","Persona amb dificultats per adaptar-se al seu medi social (aéllament, no respecta els lémits normatius ni socials del seu entorn).",entornSocial));		
		...
		situacioSocial = situacioSocialRepository.save(new SituacioSocial ("Desatencié institucional  (S.4)","Persones que per la seva situació legal o adminsitrativa no tenen accés a determinats recursos de l'émbit de  salut, educacié i protecció social.",entornSocial));		
		...
		factor=factorRepository.save(new Factor ("Es preveu que el déficit déautonomia de la persona séagreugi en un curt termini (durant léany)"));
		...
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entornAutonomia));
		factor=factorRepository.save(new Factor ("La situació i/o caracteréstiques del domicili agreugen el déficit déautonomia  de les persones que hi viuen"));
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entornAutonomia));
		factor=factorRepository.save(new Factor ("La persona amb déficit déautonomia té responsabilitats/carregues familiars"));
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entornAutonomia));
		
		
		// Proteccio
		factor=factorRepository.save(new Factor ("Una o més persones de la famélia disposa d'ingressos estables (pensions, prestacions, rendes, etc.)."));
		...
		factor=factorRepository.save(new Factor ("Una o més persones de la famélia disposa d'estalvis"));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornHabitatge));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornEconomic));
		factor=factorRepository.save(new Factor ("Una o més persones de la famélia disposa de patrimoni a part de la primera residéncia"));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornHabitatge));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornEconomic));
		factor=factorRepository.save(new Factor ("Una o més persones de la famélia disposa de treball estable."));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornHabitatge));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornEconomic));
		...
		// Proteccio
		factor=factorRepository.save(new Factor ("La persona mostra una vinculacié relacional positiva amb la famélia extensa o les seves xarxes déamics i relacions"));
		...
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornEscolar));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornFamiliar));
		factor=factorRepository.save(new Factor ("La persona/famélia és usuéria de recursos/serveis adreéats a cobrir la necessitat en léémbit relacional (escolar, familiar o comunitari) "));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornEscolar));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entornFamiliar));


		// Risc
		factor=factorRepository.save(new Factor ("Existeix una xarxa social (veéns, amics, famélia extensa..) que exerceix una influéncia negativa"));
		...
		factor=factorRepository.save(new Factor ("La persona i / o famélia té consciencia i accepta  les dificultats / malaltia causants de la seva situació"));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entorn));
		factor=factorRepository.save(new Factor ("La persona i / o famélia accepta el suport i l'orientació professional"));
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entorn));
		...
		factorGravetatRepository.save(new FactorGravetat(proteccioGravetat,factor,entorn));
		factor=factorRepository.save(new Factor ("La persona i / o famélia no té consciencia i/o no accepta  les dificultats/malaltia causants de la seva situació"));
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entorn));
		factor=factorRepository.save(new Factor ("La persona i / o famélia no accepta el suport i l'orientació professional"));
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entorn));
		...
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entorn));
		factor=factorRepository.save(new Factor ("Composicié familiar vulnerable (monoparental, famélies nombroses de categoria especial)"));
		factorGravetatRepository.save(new FactorGravetat(riscGravetat,factor,entorn));
